 ...
 * Apple Music 搜索页兜底链接。
 *
 * 关键：deepLink 必须用 **HTTPS Universal Link**，不能用 `music://` 自定义 scheme。
 *   - HTTPS：iOS 通过 `applinks:music.apple.com` 路由到 Apple Music App 的
 *     `application(_:continue:restorationHandler:)`，App 会解析 `?term=` 进搜索页；
 *     未装 Apple Music 时自动回落到 Safari 打开同一页面。
 *   - `music://music.apple.com/.../search?term=...`：iOS 只会按 scheme 把 App 唤起，
 *     Apple Music 的 custom-scheme handler **不解析 search 路径**，结果就是
 *     "App 起来了，但停在上次的首页"——也就是用户报的「点苹果按钮只拉起首页」。
 *   单曲详情页两种形式都行（见 resolveApple()），但 search 必须 HTTPS。
 */
function appleSearchFallback(title: string, artist: string, q: string): ResolvedSong {
  const enc = encodeURIComponent(q);
  const universal = `https://music.apple.com/cn/search?term=${enc}`;
  return {
    platform: 'apple',
    deepLink: universal,
    webUrl: universal,
